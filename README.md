...
```lua
{
  ...
  keymaps = {
    ...
    unfold_all = 'E',
    fold_reset = 'R',
    -- Open menu for dynamically changing the filtering options
    -- changes the filterlist for the selected filetype first if there is one set
    filter_menu = 'f',
...
To dynamically filter the displayed symbols use the filtermenu keybind in the outline-window.
Changing a single entry there toggles its filterstatus, all entries can be selected using "all" and deselected
by selecting "none".
